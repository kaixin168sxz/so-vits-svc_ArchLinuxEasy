...
def tts_fn(_text, _gender, _lang, _rate, _volume, output_format, sid, vc_transform, auto_f0,cluster_ratio, slice_db, noise_scale,pad_seconds,cl_num,lg_num,lgr_num,f0_predictor,enhancer_adaptive_key,cr_threshold, k_step,use_spk_mix,second_encoding,loudness_envelope_adjustment,progress=gr.Progress(track_tqdm=True)):
    ...
    try:
        ...
        if _lang == "Auto":
            ...
            subprocess.run([r"python3", "tts.py", _text, _lang, _rate, _volume, _gender])
        else:
            subprocess.run([r"python3", "tts.py", _text, _lang, _rate, _volume])
        target_sr = 44100
...
def dataset_preprocess(encoder, f0_predictor, use_diff, vol_aug, skip_loudnorm, num_processes, tiny_enable):
    if precheck_ok:
        ...
        preprocess_commands = [
            r"python3 resample.py %s" % (skip_loudnorm_arg),
            r"python3 preprocess_flist_config.py --speech_encoder %s %s %s" % (encoder, vol_aug_arg, tiny_arg),
            r"python3 preprocess_hubert_f0.py --num_processes %s --f0_predictor %s %s" % (num_processes ,f0_predictor, diff_arg)
            ]
        accumulated_output = ""
        #清空dataset
...
def regenerate_config(encoder, vol_aug, tiny_enable):
    ...
    vol_aug_arg = "--vol_aug" if vol_aug else ""
    tiny_arg = "--tiny" if tiny_enable else ""
    cmd = r"python3 preprocess_flist_config.py --speech_encoder %s %s %s" % (encoder, vol_aug_arg, tiny_arg)
    output = ""
    try:
...
def training(gpu_selection, encoder, tiny_enable):
    ...
    cmd = r"export CUDA_VISIBLE_DEVICES=%s && python3 train.py -c configs/config.json -m 44k" % (gpu_selection)
...
def kmeans_training(kmeans_gpu):
    if not os.listdir(dataset_dir):
        return "数据集不存在，请检查dataset文件夹"
    cmd = r"python3 cluster/train_cluster.py --gpu" if kmeans_gpu else r"python3 cluster/train_cluster.py"
    subprocess.Popen(['bash', '-c', cmd])
...
def index_training():
    if not os.listdir(dataset_dir):
        return "数据集不存在，请检查dataset文件夹"
    cmd = r"python3 train_index.py -c configs/config.json"
    subprocess.Popen(['bash', '-c', cmd])
...
def diff_training(encoder, k_step_max):
    ...
    subprocess.Popen(['bash', '-c', r"python3 train_diff.py -c configs/diffusion.yaml"])
...
with app:
        ...
        with gr.TabItem("小工具/实验室特性"):
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("歌曲人声分离"):
                    ...
                    """)
                    with gr.Row():
                        input_audio = gr.Audio(label="上传原始音频", type="filepath", sources="upload")
    ...
    with gr.Tabs():
        ...
        app.queue(max_size=2044, concurrency_count=1022).launch(server_name="127.0.0.1",inbrowser=True,quiet=True)
